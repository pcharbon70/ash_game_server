...
**Ash Game Server** is a multiplayer game server implementing Entity Component System (ECS) architecture using the Ash Framework. The project creates a high-performance, scalable game server that combines:

- **ECS Architecture**: Entities as lightweight IDs, Components as data stored in ETS tables, Systems as concurrent game logic
- **Jido Agents**: Autonomous AI agents for complex NPC behaviors and distributed game systems
- **Ash Framework**: Declarative domain modeling, real-time subscriptions, and PostgreSQL persistence
- **Spark DSL**: Custom domain-specific language for defining game components, systems, and rules
- **Hybrid Storage**: ETS tables for ultra-fast in-memory game state + PostgreSQL for durability

The server is designed to handle 1000+ concurrent players with sub-millisecond component access times while maintaining fault tolerance and real-time synchronization.
